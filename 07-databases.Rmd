...
Caution is warranted, however, given the nature of the default `join` performance (inner) since ERQ and PANAS have a different number of items. Briefly, an 'inner' join returns the requested values from where there is a match in both tables while a 'left' join returns all records from the first table even if no matching record is found in the second (see [here](https://www.w3schools.com/sql/sql_join.asp) for some helpful diagrams). Accordingly, we will use `tbl_panas.item_panas` to create the new `item_num` column and a `left` join:
...
import pymysql
import paramiko
from sshtunnel impport SSHTunnelForwarder
...
To interact with a MySQL database locally, we first start by establishing a connection between R and MySQL. Start a new R file, load the `RMySQL` library, and setup a connection via [`dbConnect`](https://www.rdocumentation.org/packages/DBI/versions/1.2.0/topics/dbConnect) that specifies we are using the [`MySQL`](https://rdrr.io/cran/RMySQL/man/dbConnect-MySQLDriver-method.html) driver:
...
Here, I have specified an [SSH](https://www.digitalocean.com/community/tutorials/how-to-use-ssh-to-connect-to-a-remote-server) connection and supplied a number of [options](https://man7.org/linux/man-pages/man1/ssh.1.html). Specifically, I supply my [RSA key](https://www.digitalocean.com/community/tutorials/how-to-configure-ssh-key-based-authentication-on-a-linux-server) for labarserv2 (available in the OS via `$RSA_LS2`) with the `-i` option, I bind my local port `3308` with the default MySQL IP of `127.0.0.1` and MySQL port of `3306` via the `-L` option. Finally, the `-N` option is useful for port forwarding as a remote command is not executed.
...
One method of inserting data into a table is by building an `insert` object with [`sqlInterpolate`](https://dbi.r-dbi.org/reference/sqlInterpolate.html) and then passing this object to [`dbSendQuery`](https://www.rdocumentation.org/packages/RMySQL/versions/0.9-3/topics/dbSendQuery-methods). The function `sqlInterpolate` takes three arguments: the connection object, a SQL command, and data. As we have the connection object saved as `db_con` (above), we can next specify a SQL [insert](#insert_data) command that uses `?val` to identify the variable `val`:
...
To specify data (the third argument of `sqlInterpolate`), we create a [named list](https://www.r-tutor.com/r-introduction/list/named-list-members) which uses the SQL variables from the insert command (val1-4). Doing this for a single row of data (`df_erq[1, ]`), we can assign each `df_erq` column to their respective variable:
...
Finally we build our insert object with `sqlInterpolate`, and pass that object to `dbSendQuery`. The function of `dbSendQuery` is to submit and the execute whatever statement is supplied as the second argument, here our `insert` object.
...
With this simple insertion method working, we can loop through the dataframe to iteratively `insert` data into `tbl_erq`. Here we submit data row-by-row:
...
, which has the added benefit of a simplified specification. For instance, we can add the first row of `df_erq` to the table `tbl_erq` via :
...
When working in larger workflows and with complicated selection commands, it might be useful to save SQL syntax as its own file and then read said file when building the query object. For instance, the following statement joins the `tbl_erq` and `tbl_panas`, while also converting from long to tidy format:
...
This SQL file (saved at `${HOME}/dl_erq.sql`) can then be read by the [`readr`](https://cran.r-project.org/web/packages/readr/index.html) package, and incorporated into the `dbSendQuery` command:
