...
Python can be used to connect to the MySQL server both locally (labarserv2) and remotely (e.g. from the Duke Compute Cluster). A number of packages are available to support this, but here we will focus on [mysql-connector-python](https://dev.mysql.com/doc/connector-python/en/connector-python-installation-binary.html) (local; also, [here](https://pypi.org/project/mysql-connector-python/)) and [pymysql](https://pypi.org/project/pymysql/) (remote) as they seem to be most stable at the time of this writing. By utilizing Python, we can automate database management and data pulls.
...
Given that the MySQL server exists on labarserv2, it may not always be feasible to login to labarserv2 to execute SQL queries. For instance, large analyses may require time or resources that are not reasonable for labarserv2. In this case, processes would be conducted on the Duke Compute Cluster (DCC) but access to the database is still necessary, and manually exporting dataframes to copy to the DCC is an inefficient workflow. Another example, again involving the DCC, involves updating the database with output computed on the DCC. It would likewise be inefficient to manually copy files from the DCC to labarserv2 just to update the database. 

Practically speaking, it is more common to interact with a SQL database over the network (remotely), such as is the case when a researcher is working on their personal/work machine with data that exists on a remote server. Here we will describe setting up an [SSH Tunnel](https://www.ssh.com/academy/ssh/tunneling) that will allow us to connect to the remote server.
...
A cursor can the be established in the same way as above:
...
Inserting data into tables can be accomplished by the cursor's [`execute`](https://dev.mysql.com/doc/connector-python/en/connector-python-api-mysqlcursor-execute.html) method, which requires two arguments -- the operation (SQL syntax) and a tuple of data. Accordingly, we ready for data insertion by building the first argument, and specify a MySQL variable via `%s` in the `values` field rather than actual data:
...
And finally we upload the entire dataset via `executemany`:
...
As with inserting data (above), given a connection and cursor established we can begin specifying SQL commands for execution. We'll start by selecting the first 10 rows of `tbl_erq`. To do this, we can assign a python variable to hold the SQL syntax as a string, and then incorporate the syntax in the cursor's [execute](https://dev.mysql.com/doc/connector-python/en/connector-python-api-mysqlcursor-execute.html) method:
